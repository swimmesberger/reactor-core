 ...
 * @author Simon Baslé
 */
final class BoundedFacadeElasticScheduler
	...
	static final AtomicIntegerFieldUpdater<BoundedFacadeElasticScheduler> REMAINING_THREADS =
			AtomicIntegerFieldUpdater.newUpdater(BoundedFacadeElasticScheduler.class, "remainingThreads");

	volatile int                                                   remainingDeferredTasks;
	static final AtomicIntegerFieldUpdater<BoundedFacadeElasticScheduler>REMAINING_DEFERRED_TASKS =
			AtomicIntegerFieldUpdater.newUpdater(BoundedFacadeElasticScheduler.class, "remainingDeferredTasks");


	BoundedFacadeElasticScheduler(int threadCap, int deferredTaskCap, ThreadFactory factory, int ttlSeconds) {
		if (ttlSeconds < 0) {
	...
	/**
	 * Instantiates the default {@link ScheduledExecutorService} for the BoundedFacadeElasticScheduler
	...
	static final class CachedService implements Disposable, Scannable {
		...
		final BoundedFacadeElasticScheduler  parent;
		final ScheduledExecutorService exec;

		CachedService(@Nullable BoundedFacadeElasticScheduler parent) {
			this.parent = parent;
			if (parent != null) {
	...
	static final class DeferredWorker extends ConcurrentLinkedQueue<DeferredWorkerTask> implements Worker, Scannable,
		...
		DeferredWorker(BoundedFacadeElasticScheduler parent) {
			this.parent = parent;
	...
	static final class DeferredDirect extends AtomicReference<CachedService> implements Scannable, Disposable,
		...
		DeferredDirect(Runnable task, long delay, long period, TimeUnit unit, BoundedFacadeElasticScheduler parent) {
			this.task = task;
			this.delay = delay;
			this.period = period;
			this.timeUnit = unit;
